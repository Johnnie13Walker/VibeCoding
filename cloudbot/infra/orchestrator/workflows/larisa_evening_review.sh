...
REPORT_FILE="$REPORT_DIR/larisa_evening_review_${STAMP}.txt"

args=(python3 -m agents.larisa_ivanovna --command get_evening_review)
...
log "larisa_evening_review: старт"
...
if [[ "$rc" -ne 0 ]]; then
  log "larisa_evening_review: ошибка (rc=${rc}), отчет=${REPORT_FILE}"
  tail -n 120 "$REPORT_FILE" || true
  exit "$rc"
fi

log "larisa_evening_review: успешно, отчет=${REPORT_FILE}"
