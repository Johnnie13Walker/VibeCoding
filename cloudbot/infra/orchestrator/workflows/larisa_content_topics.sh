...
REPORT_FILE="$REPORT_DIR/larisa_content_topics_${STAMP}.txt"

command_name="get_content_topics"
if [[ "$TOPIC_INDEX" -gt 0 ]]; then
  command_name="get_content_post"
fi

args=(python3 -m agents.larisa_ivanovna --command "$command_name" --period "$PERIOD")
if [[ "$TOPIC_INDEX" -gt 0 ]]; then
  args+=(--topic "$TOPIC_INDEX")
fi
...
log "larisa_content_topics: старт (period=${PERIOD}, topic=${TOPIC_INDEX})"
...
if [[ "$rc" -ne 0 ]]; then
  log "larisa_content_topics: ошибка (rc=${rc}), отчет=${REPORT_FILE}"
  tail -n 120 "$REPORT_FILE" || true
  exit "$rc"
fi

log "larisa_content_topics: успешно, отчет=${REPORT_FILE}"
