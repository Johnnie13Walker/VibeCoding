...
REPORT_FILE="$REPORT_DIR/larisa_midday_replan_${STAMP}.txt"

args=(python3 -m agents.larisa_ivanovna --command get_midday_replan)
...
log "larisa_midday_replan: старт"
...
if [[ "$rc" -ne 0 ]]; then
  log "larisa_midday_replan: ошибка (rc=${rc}), отчет=${REPORT_FILE}"
  tail -n 120 "$REPORT_FILE" || true
  exit "$rc"
fi

log "larisa_midday_replan: успешно, отчет=${REPORT_FILE}"
