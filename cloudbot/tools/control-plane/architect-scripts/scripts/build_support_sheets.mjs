...
function buildQualityValues(cohort, wins) {
  ...
  const values = [
    ...
    ["Период", formatPeriod(cohort.meta), "", "Обновлено", moscowTimestamp(), "", "Логика", "Только сделки, проходившие воронку продаж"],
...
function buildMethodologyValues() {
  return [
    ...
    ["Обращение", "Сделка создана в периоде. Для когортного слоя строка месяца = месяц создания сделки."],
    ["Лид", "Первый вход сделки в стадию «Подготовка БРИФа»."],
    ...
    ["Источник", "Берётся из карточки сделки на момент выгрузки. Внутри отчёта он не пересчитывается при переносах между воронками."],
