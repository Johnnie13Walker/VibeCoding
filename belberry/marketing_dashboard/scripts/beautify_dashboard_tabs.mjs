...
function buildSourcesGrid(sourceRows) {
  ...
  putRow(grid, 1, 0, ["Обычно сделка относится к месяцу её создания. Для «Телемаркетинг» месяц берётся по полю «Дата встречи», источник подтверждается встречей от отдела телемаркетинга."]);
  putRow(grid, 3, 0, ["Общая сводка"]);
...
const dynamicsGrid = buildMonthlyBrandGrid("Динамика по месяцам · Когортный слой", "Месяц строки = месяц создания сделки; для «Телемаркетинг» месяц = поле «Дата встречи». Сводка сверху показывает общий объём по брендам и хвост без бренда.", cohortMap, cohortMap.get("Без бренда"));
