...
function buildBrandTab(brand, rows, periodLabel) {
  ...
  putRow(grid, 1, 0, [`Период: ${periodLabel}. Обычно строка месяца = месяц создания сделки; для «Телемаркетинг» месяц берётся по полю «Дата встречи». Метрики справа показывают текущий срез когорты на дату обновления.`]);
...
function buildDealsGrid(rows, periodLabel) {
  ...
  putRow(grid, 0, 0, ["Сделки и встречи отчётного контура"]);
  putRow(grid, 1, 0, [`Логика: сделка учитывается, если в ${periodLabel} проходила через воронку продаж. Для «Телемаркетинг» дополнительно учитываются сделки со встречей от отдела телемаркетинга, месяц = поле «Дата встречи».`]);
