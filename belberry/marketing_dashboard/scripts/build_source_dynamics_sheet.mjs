...
function buildGrids(data) {
  ...
  row(["Логика", "Источник берётся из Bitrix24 SOURCE_ID, промапленного через crm.status.list. Для сделок со встречей от отдела телемаркетинга источник считается как «Телемаркетинг», даже если до марта 2026 сделка пришла из лидов, а не из воронки «Телемаркетинг». Raw SOURCE_ID=Телемаркетинг без встречи от телемаркетолога уходит в «Не выяснено». В телемаркетинге обращение считается по полю «Дата встречи» первой встречи, а лидом считается сделка, где первая встреча проведена или перенос первой встречи завершился проведённой встречей. Сделки с причиной отказа «Спам» или «Вход: нет связи» / «Нет связи» остаются обращениями, но не считаются лидами."]);
  row([]);
