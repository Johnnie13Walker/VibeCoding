...
function buildGrids(data) {
  ...
  row(["Логика", "Учитываются сделки отчётного контура, у которых причина отказа в Bitrix24 равна «Спам» или «Вход: нет связи» / «Нет связи» без учёта регистра и лишних пробелов. В рабочих вкладках они остаются обращениями, но не считаются лидами. Для «Телемаркетинг» месяц берётся по полю «Дата встречи», для остальных источников — по дате создания/первого события сделки."]);
