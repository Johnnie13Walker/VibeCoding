...
from crm_company_enrich.stages import deal_reactivation_count_field as stage
...
def _bitrix_integer_field(**overrides):
    field = {
        "ID": "88",
        "FIELD_NAME": "UF_CRM_REACTIVATION_COUNT",
        "USER_TYPE_ID": "integer",
        "XML_ID": "UF_CRM_REACTIVATION_COUNT",
        "SHOW_FILTER": "E",
        "SHOW_IN_LIST": "N",
        "EDIT_IN_LIST": "N",
        "SETTINGS": {
...
def test_dry_run_creates_integer_payload():
    ...
    assert summary["payload"]["FIELD_NAME"] == "UF_CRM_REACTIVATION_COUNT"
...
def test_existing_correct_field_is_noop():
    summary = stage.run(FakeBitrix([_bitrix_integer_field()]))
...
def test_existing_wrong_type_string_triggers_update():
    field = stage._field_payload() | {"ID": "88", "USER_TYPE_ID": "string"}
    bx = FakeBitrix([field])
...
def test_verify_accepts_response_without_labels():
    assert stage.verify_field(_bitrix_integer_field())["ok"] is True


def test_verify_accepts_bitrix_default_size_and_max_value():
    assert stage.verify_field(
