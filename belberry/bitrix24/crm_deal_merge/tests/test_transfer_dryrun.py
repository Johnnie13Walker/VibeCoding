...
def test_transfer_dryrun_does_not_call_write_methods(capsys) -> None:
    ...
    # dry-run может звать ensure_sheet (idempotent create-if-missing) — это не "write",
    # а защита от 400 на пустом портале. Реальные write-операции в sheets
    # (update/append) должны отсутствовать.
    write_only = [c for c in sheets.write_calls if c not in ("ensure_sheet",)]
    assert write_only == []
