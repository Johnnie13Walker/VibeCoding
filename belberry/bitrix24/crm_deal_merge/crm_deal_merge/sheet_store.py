...
def read_groups(sheets: SheetsClient) -> list[tuple[int, Group]]:
    # GROUP_HEADERS = 29 колонок (выходит за Z). Явно расширяем range до AZ.
    rows = sheets.read(TAB_GROUPS, "A1:AZ10000")
    if not rows:
        return []
...
def update_all_groups(sheets: SheetsClient, groups: list[Group]) -> None:
    if not groups:
        return
    end_col = _col_name(len(GROUP_HEADERS))
    end_row = len(groups) + 1
    sheets.update(
        TAB_GROUPS,
        f"A2:{end_col}{end_row}",
        [group.to_sheet_row() for group in groups],
        value_input_option="USER_ENTERED",
    )
...
def read_inventory(sheets: SheetsClient) -> tuple[list[str], list[tuple[int, dict[str, str]]]]:
    ensure_inventory(sheets)  # create если нет — иначе read 400
    # Inventory уже превышает 10k строк; default range Sheets API может
    # обрезать хвост, что опасно для transfer/skip нижних строк.
    rows = sheets.read(TAB_INVENTORY, "A1:I50000")
    if not rows:
