...
Каждая задача:
- title — короткий императив В ИНФИНИТИВЕ («Отправить…», «Дожать…», «Оценить…»), с
  КОНКРЕТИКОЙ из этой встречи; БЕЗ обращения на «ты»/«вы» («Отправь», «Дождись» — нельзя),
  без канцелярита и пересказа. Плохо: «Получить обратную связь». Хорошо: «Дожать клиента
  по смете на контекст — обещал ответ к среде».
...
- deadline — КОРОТКОЙ формой, понятной автопарсеру: «завтра», «послезавтра»,
  «через N дней», «через неделю», «через две недели», день недели («к среде», «в пятницу»)
  или конкретная дата. БЕЗ «около/плюс-минус/в течение этой недели точно» и без целых
  фраз. Если срок на встрече не обсуждали — null.
