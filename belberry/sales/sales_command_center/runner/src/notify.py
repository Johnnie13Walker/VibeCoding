...
def send_report_link(report_date: str | date, *, http_post: HttpPost | None = None) -> bool:
    token = _env("TELEGRAM_BOT_TOKEN", "SCC_TELEGRAM_BOT_TOKEN")
    chat_id = _env("TELEGRAM_CHAT_ID", "SCC_TELEGRAM_CHAT_ID")
    base_url = _env("SCC_BASE_URL")

    if not token or not chat_id or not base_url:
        LOGGER.warning("Telegram report link skipped: missing env")
        return False

    date_text = html.escape(_report_date(report_date))
    url = f"{base_url.rstrip('/')}/day/{date_text}"
    text = f'Сводка продаж за {date_text} готова: <a href="{html.escape(url, quote=True)}">открыть</a>'
...
def send_alert(
    ...
    *,
    ...
    text = f"⚠️ Сбой генерации сводки{date_part}: {html.escape(message)}"
