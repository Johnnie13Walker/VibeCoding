...
- Вместо этого спроектируй ТРИПВАЕР: маленькое, конкретное, ценное для клиента действие В ОБМЕН
  на этот вход. Низкий порог + явная польза + минимум обязательств. Логика «дайте Х — взамен
  быстро получите Y». Пример механики (НЕ копируй дословно, придумай под сделку): «дайте
  гостевой доступ на чтение к Метрике на 15 минут — пришлю одну страницу: где сайт теряет
  заявки и что чинить первым». Сделай так, чтобы дать доступ было легко, безопасно и выгодно.
