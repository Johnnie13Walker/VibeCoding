...
- Цитату клиента бери только дословно из транскрипта. Если показательной цитаты нет,
  client_quote = null.
- Не верь статусу Bitrix слепо. ВАЖНО: stageId встречи (напр. DT1048_24:SUCCESS) значит
  лишь, что встреча СОСТОЯЛАСЬ (проведена) — это бизнес-процесс ВСТРЕЧИ, а НЕ статус
  сделки и НЕ её выигрыш/оплата. У КАЖДОЙ разобранной встречи stageId=SUCCESS, поэтому
  НЕ делай из него вывод об «успехе сделки» и не пиши «статус сделки SUCCESS».
  status_discrepancy=true ставь ТОЛЬКО при реальном расхождении: суть разговора
  (клиент не дал согласия/обязательства) противоречит тому, как РЕЗУЛЬТАТ подан в
  итогах/CRM — а не из самого факта, что встреча проведена.
- Поставь встрече итоговый балл score — целое 1..10. Опирайся на чек-лист, силу
  следующего шага, аргументацию и обязательство клиента (commitment). Шкала:
  * 10 — образцовая встреча (редкий балл): весь чек-лист ✅, возражения закрыты,
    вскрыты бюджет + ЛПР + сроки решения, клиент ВЗЯЛ обязательство и сам инициирует
    следующий шаг, договорённость с датой зафиксирована (письменно/прямо в разговоре).
  * 9 — весь чек-лист ✅, аргументация цифрами, клиент взял обязательство (не «подумает»),
    следующий шаг с датой; не хватает лишь части «исключительного» (напр. ЛПР или сроки
    решения вскрыты не полностью).
  * 8 — сильная встреча: чек-лист почти весь ✅ (≤1 пункт ⚠️), следующий шаг есть, но
    обязательство мягкое («подумает») ИЛИ упущен один ключевой пункт (бюджет/ЛПР/кейсы).
  * 6-7 — рабочая, но с провисами: 2-3 пункта ⚠️/❌, следующий шаг размытый.
  * 4-5 — слабая: анкета вместо диалога ИЛИ нет бюджета / кейсов / следующего шага.
  * 1-3 — провал: клиент потерян, нет потребности и следующего шага, грубые ошибки.
  Эталоны: тяжёлый брифинг без бюджета ≈5-6; крепкая защита с мягким обязательством ≈8.
...
- cases_mentioned — кейсы/референсы (примеры работ Belberry для ДРУГИХ клиентов),
  которые РЕАЛЬНО приводили на встрече. Для каждого: client (ниша или имя клиента, как
  назвали — напр. «стоматология в Казани», «сеть клиник»), service (услуга, если ясно:
  SEO/ORM/Контекст/Веб-разработка/GEO/SMM/Аналитика; иначе null), result (результат или
  цифры, если назвали; иначе null). ТОЛЬКО реально упомянутое, ничего не выдумывай.
  Если кейсы на встрече не показывали — [].
