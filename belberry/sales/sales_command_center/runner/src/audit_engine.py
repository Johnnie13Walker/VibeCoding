...
from . import analyze_llm, bx_client, call_audio, web_context
...
def collect_deal_context(deal_id: int) -> dict[str, Any] | None:
    ...
    external = None
    try:  # внешний контекст (сайт клиента) — строго fail-safe, не валит аудит
        external = web_context.deal_external_context(deal)
    except Exception:
        external = None
