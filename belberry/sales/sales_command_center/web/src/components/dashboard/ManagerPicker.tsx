...
export interface PickerManager {
  managerId: number;
  name: string;
  /** false → показываем тег «уволен». По умолчанию активен. */
  isActive?: boolean;
}
...
/** Выпадающий чек-лист менеджеров — единый мультиселектор для всех блоков дашборда. */
export function ManagerPicker({
  managers, selected, onChange,
}: {
  managers: PickerManager[];
  selected: Set<number>;
          ...
          {managers.map((m) => {
            ...
            return (
              ...
              >
                ...
                {m.isActive === false ? <FiredTag /> : null}
              </button>
            );
          })}
        </div>
