    ...
    <div>
      ...
      {n === 0 || data.hours.length === 0 ? (
        <p style={{ color: 'var(--bb-muted)' }}>
          {n === 0 ? 'Выберите звонарей в фильтре, чтобы увидеть карту набора.' : 'Нет данных о наборах за период.'}
        </p>
      ) : (
        <>
          ...
          <p style={{ fontSize: 12, color: 'var(--bb-faint)', marginTop: 12 }}>
            Сколько наборов делают звонари по часам (МСК) и дням недели · за 3 месяца · всего {nf(grid.totalDials)} наборов по выбранным. Темнее — больше звонят. Выбери одного звонаря — увидишь только его.
          </p>
        </>
      )}
    </div>
