...
function tmTask(dealTitle: string, n: AuditResult['narrative']): { title: string; description: string } {
  const ctx = n?.summary ? n.summary.slice(0, 160) : '';
  return {
    title: `Прозвонить ${dealTitle} — реактивировать и назначить встречу`,
    description: [
      'Сделка возвращена в телемаркетинг. Задача — заново зацепить клиента и НАЗНАЧИТЬ ВСТРЕЧУ с менеджером отдела продаж. Глубоко не квалифицировать: бюджет и детали выяснит менеджер на встрече.',
      ctx ? `О чём была сделка: ${ctx}` : '',
      '1. Дозвониться, по-доброму переоткрыть диалог: напомнить про прошлое обращение по проекту.',
      '2. Коротко вернуть интерес — «хотим показать свежие идеи / что можем сделать по вашей задаче».',
      '3. Цель звонка — договориться о короткой встрече (20–30 мин) со специалистом. Предложить 2 слота на выбор.',
      '4. Назначить встречу и зафиксировать дату/время. Если сейчас не готов — взять согласие перезвонить и поставить дату следующего касания.',
    ].filter(Boolean).join('\n'),
  };
}
