...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  for (const engine of ENGINE_CANDIDATES) {
    const file = path.join(engine, 'clients', jobDirName(job.id, job.dealId), 'deck.pdf');
    if (existsSync(file)) {
      return new Response(readFileSync(file), {
        headers: {
          'Content-Type': 'application/pdf',
          'Content-Disposition': 'inline; filename="kp-draft.pdf"',
          'Cache-Control': 'no-store',
        },
      });
    }
  }
  return new Response('PDF не найден — пересоберите задание (нужен Chrome на сервере)', { status: 404 });
}
