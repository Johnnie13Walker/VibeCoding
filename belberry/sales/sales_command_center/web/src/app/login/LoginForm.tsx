...
export function LoginForm({ redirectTo }: LoginFormProps) {
  ...
  return (
    <section className="w-full max-w-sm rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
      <div className="mb-6">
        <p className="text-sm font-medium text-slate-500">Global Sales Dashboard</p>
        <h1 className="mt-2 text-2xl font-semibold text-slate-950">Вход по Bitrix24</h1>
      </div>
      ...
      {step === 'email' ? (
        ...
        <form className="grid gap-4" onSubmit={verify}>
          <label className="grid gap-2 text-sm font-medium text-slate-700">
            Код из Bitrix24
            <input
              className="rounded-md border border-slate-300 px-3 py-2 tracking-widest outline-none focus:border-slate-900"
