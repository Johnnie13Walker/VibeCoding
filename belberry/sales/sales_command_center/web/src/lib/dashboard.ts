...
export async function getDashboardData(range: Period = 'month'): Promise<DashboardData> {
  ...
  if (salesIds.length) {
    const mtMgrRows = await db
      ...
      .from(meetings)
      .where(and(eq(meetings.status, MEETING_HELD_STAGE), sql`date(${meetings.scheduledAt} at time zone 'Europe/Moscow') = ${meetings.reportDate}`, gte(meetings.reportDate, start), lte(meetings.reportDate, end), inArray(meetings.managerId, salesIds)))
  ...
  const dynMeetRows = await db
    ...
    .from(meetings)
    .where(and(eq(meetings.status, MEETING_HELD_STAGE), sql`date(${meetings.scheduledAt} at time zone 'Europe/Moscow') = ${meetings.reportDate}`, gte(meetings.reportDate, dynStart)))
