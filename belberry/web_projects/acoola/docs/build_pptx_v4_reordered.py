...
def main():
    ...
    builders = [
        # Акт I — Hook + Why (1–3): проблема → манифест как ответ
        # «Три сдвига digital» удалены — дублировали «Что сломалось»
        slide_01_cover_provocation,    # 1 — обложка
        slide_04_broken,               # 2 — что сломалось
        slide_02_manifest,             # 3 — манифест ПОСЛЕ проблемы
        # Акт II — Наша модель (4–7)
        slide_06_long_term,            # 4 — 3,5+ года
        slide_07_95_visualization,     # 5 — 95 из 100
        slide_08_anti_marketing,       # 6 — стоп-лист
        slide_09a_weak_spots,          # 7 — где слабые
        # Акт III — Proof: кейсы → провал → клиенты → команда (8–14)
        slide_case_teo,                # 8
        slide_11_story_wwerb,          # 9
        slide_case_kenner_context,     # 10
        slide_case_orm_security,       # 11
        slide_13_failure_case,         # 12 — провал между кейсами и клиентами
        slide_12_clients,              # 13
        slide_13_team_quote,           # 14
        # Акт IV — о компании (15): «История 2019–2025» удалена (корпоративная вставка без пользы)
        slide_countries,               # 15 — география
        # Акт V — закрытие: страх → цена → услуги → прайс → награды → GEO → финал (16–23)
        slide_5_fears,                 # 16 — 5 страхов
        slide_17_cost_of_mistake,      # 17 — цена ошибки
        slide_services_lifecycle_map,  # 18 — карта услуг
        slide_pricing,                 # 19 — прайс
        slide_14_awards,               # 20 — награды как social proof перед кульминацией
        slide_09_geo_climax,           # 21 — GEO как кульминация в финале
        slide_18_close_provocation,    # 22 — финал «один контракт или пять»
        slide_partnership,             # 23 — партнёрка
    ]
    for b in builders:
        b(prs)
    ...
    out = "/Users/pro2kuror/Desktop/Cloude/docs/acoola_marketing_kit_v4_reordered.pptx"
    prs.save(out)
