...
- При ручном обогащении строк из `Телемаркетинг без реквизитов` бренд проекта
  и сфера деятельности обязательны на двух уровнях: company + связанная C50
  deal. Перед удалением строки нужен read-back обоих объектов; удалять через
  `delete-sheet-row-guarded`, не прямым `deleteDimension`.
