# GD-324: краткое summary по аудиту полей Лидов и Сделок

Подготовлено: `2026-04-09 14:37 МСК`
...
- В фокус взяты только `Лиды` и все категории `Сделок`, без смарт-процессов.

## Что плохо

- Всего проанализировано `3029` строк полей по `Лидам` и всем воронкам `Сделок`.
...
- Найдено минимум `2686` строк по смысловым дублям.
- Найдено `21` кандидатов на стандартизацию.
...
- В этой версии собран полный мета-аудит по полям `Лидов` и `Сделок`. Полный factual sweep по всем live-записям требует отдельного фонового запуска из-за объема REST-пагинации.
