...
const RANGES = [
  ...
  ["deals", "'Сделки'!A1:H260"],
  ["quality", "'Качество данных'!A1:I120"],
  ["noBrand", "'Без бренда'!A1:H40"],
  ...
  ["rawSpamSources", "'RAW · spam_sources_2026'!A1:H120"],
  ["winsDeals", "'Продажи YTD 2026 · сделки'!A1:P45"],
  ["winsServices", "'Продажи YTD 2026 · услуги'!A1:R55"],
...
expectAtMost("Спам-фильтр / created delta не больше найденного спама", Number(spamFilter.created_deals_before || 0) - Number(spamFilter.created_deals_after || 0), spamFilter.spam_deals_found);
expectAtMost("Спам-фильтр / sales-only delta не больше найденного спама", Number(spamFilter.sales_only_created_before || 0) - Number(spamFilter.sales_only_created_after || 0), spamFilter.spam_deals_found);
expectEqual("Спам-фильтр / event candidates delta", Number(spamFilter.event_candidates_before || 0) - Number(spamFilter.event_candidates_after || 0), spamFilter.spam_deals_found);
...
expectEqual("Спам-фильтр / spam ids отсутствуют в детализации", (cohort.detail_rows || []).filter((row) => spamDealIds.has(String(row.id))).length, 0);
...
expectIncludes("Динамика источников 2026 / логика событийная", cell(sourceDynamics, 3, 2), "первого входа сделки в лид-стадию");
